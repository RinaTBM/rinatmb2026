...
- Kashu/Tagada card payments: server-side hosted checkout path exists on `integration/tagadapay-hosted-checkout-2026` but **UI stays off** (`VITE_KASHU_CARD_ENABLED` unset/false). Prefer Tagada hosted checkout/init over Pay V2. Do not enable live card checkout or deploy production frontend for card without owner approval. Phase 2B report: `docs/tagadapay-phase2b-remediation-report.md`.
...
- Phase 2B gotcha: catalog tax flags are off (`isTaxable=false`) and Sem $595 hosted tax is $0, but the Simple Checkout funnel still has a **ShippingRates** island that independently adds $30/$50 on top of mapped `MBM-SHIP-*` line items (and on service-only carts). Until that island/rates are disabled in the Tagada dashboard, keep `TAGADA_SHIPPING_PARITY_BLOCKER` and FE eligibility blocking `shipping_cents > 0`. Webhook amount equality must stay strict.
