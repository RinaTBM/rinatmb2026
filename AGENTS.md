...
- **GEN medication retail pricing (owner-locked):** `retail = medicationCost × 1.75 + pharmacyShippingInternal`. Customer medication shipping charge = $0 (shipping baked into retail). Do **not** use workbook `Cost +75%` as-is — that column is `landedCost × 1.75` (= `(cost+shipping)×1.75`), which is different. Multi-vial / Any Dose: price each fill with the formula for that vial. See `docs/GEN_RETAIL_PRICING_RULE.md` and `docs/GEN_FINAL_LOCKED_EXECUTION_PLAN.md`.
