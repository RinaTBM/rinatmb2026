...
- **Tagada ShippingRates must remain disabled** for this architecture. Simple Checkout (`funnelv2_834831bf5aa7` on `checkout.mybaremethod.com`) still embeds a Studio **ShippingRates** island on checkout page `checkout-5596ip` (plugin `checkout-6d366d`). Funnel API JSON does not include that island — remove it in Tagada Studio and republish. **Hiding rates (“0 of 3 visible”) is not enough:** hosted checkout still added a **$50 Shipping** line on $30/$50/IPV carts while showing “No shipping methods available” (Phase 2C v2). Until totals match MBM with no extra Tagada shipping, keep `TAGADA_SHIPPING_PARITY_BLOCKER` and FE eligibility blocking `shipping_cents > 0`. See `docs/tagadapay-phase2c-shippingrates-blocker.md`.
