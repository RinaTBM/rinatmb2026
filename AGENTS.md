...
- **GEN medication retail pricing (owner-locked, GEN-CATALOG-1I):** One-time: `raw=(cost×1.75)+shipping` → nearest $X9. **3-month:** `((monthly_cost×1.75)+shipping)×3` → $X9. **6-month:** same ×6. **SEM Any Dose membership $149/mo** and **TIR Any Dose membership $275/mo** are owner-set overrides (not auto 1.75). Do not overwrite one-time with membership. Customer med shipping included in retail. Authority: `docs/GEN_FINAL_MBM_RETAIL_PRICING.md`.
