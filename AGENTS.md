...
- Registry: `pairingVerificationRegistry.ts` → apply via `applyPairingVerification.ts`. Latest closeout: `docs/MBM_GEN_MANUAL_FIX_CLOSEOUT_2.md` (still **8/15**; CREATE not authorized). **Owner direct GEN URLs:** `docs/MBM_GEN_OWNER_DIRECT_URL_MAP.md` — use exact product links for locked CPs; TIR membership `E3MXZe…` ≠ GLP-2 `SvFDJ7…` ($279). Fix plan: `docs/MBM_GEN_FINAL_MANUAL_FIX_PLAN.md`. Click guide now includes proven direct URLs. Next creates: `docs/MBM_GEN_NEXT_CREATE_PREFLIGHT.md` (13) — do not execute.
