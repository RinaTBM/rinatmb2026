...
- **Owner working-set authority (MBM-GEN-OWNER-LINKS-AS-WORKING-SET-1):** `docs/MBM_OWNER_SELECTED_GEN_WORKING_SET.md` (+ `.json`) — the **22 owner-supplied GEN product URLs** are `OWNER_SELECTED_GEN_WORKING_SET` (active). Prior **Locked-7** Client Product list is `SUPERSEDED_BY_OWNER_DIRECT_GEN_WORKING_SET` — do not force Locked-7 as required routing architecture. Prefer REUSE of compatible owner CPs (incl. TIR `SvFDJ7…` for membership @$275; SEM membership @$149 via Any Dose backends) over creates. Website family architecture unchanged.
- **Final routing preflight (MBM-OWNER-WORKING-SET-FINAL-ROUTING-PREFLIGHT):** `docs/MBM_OWNER_SET_FINAL_ROUTING_PREFLIGHT.md` (+ `.json`). **TIR_SINGLE_BACKEND_CP_APPROVED=YES** (`SvFDJ7…` for all TIR selectors incl. membership @$275; tier CREATEs=0; repair 3-PACK before READY). **FAT_BURNER_7KIX55_REUSE=NO** (AOD-only formulary vs intended triple). Creates actually required: **7** (Wolverine Capsule, Minoxidil Dual, NAD r85, Estradiol ×4); NAD Injection = FORMULARY_PENDING (100mg/mL). READY TO BUILD FINAL WEBSITE ROUTING: YES · READY TO PUBLISH: NO. No GEN writes yet.
- Registry: `pairingVerificationRegistry.ts` → apply via `applyPairingVerification.ts`. Historical closeout: `docs/MBM_GEN_MANUAL_FIX_CLOSEOUT_2.md` (Locked-7 era **8/15**). URL inventory: `docs/MBM_GEN_OWNER_DIRECT_URL_MAP.md`. Historical fix plan: `docs/MBM_GEN_FINAL_MANUAL_FIX_PLAN.md` (superseded as working-set authority). Next creates preflight: `docs/MBM_GEN_NEXT_CREATE_PREFLIGHT.md` — re-scope against owner working set before any CREATE; do not execute yet.
