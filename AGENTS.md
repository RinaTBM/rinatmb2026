...
- Tagada credentials: keep real `TAGADA_API_KEY` (`sk_crm_…`) and `TAGADA_STORE_ID` (`store_…`) in **BSG Edge secrets**. Cursor agent env may only see Management API digests for the same names — do not treat those digests as API keys. Live discovery+sync (2026-08-11): store “My Bare Method” auth PASS; **52/52 MBM SKUs mapped** in Tagada; `tbmgroup.site` unverified; `checkout.mybaremethod.com` verified; Airwallex processor still testMode. Card checkout remains disabled. See `docs/tagada-product-mapping-review.md` and `docs/kashu-sku-map-seed.json`.
