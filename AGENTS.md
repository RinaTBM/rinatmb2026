...
- Registry: `pairingVerificationRegistry.ts` → apply via `applyPairingVerification.ts` (also promotes `GEN_PAIRING_PENDING` → `ROUTING_READY` for verified CPs only). Latest live closeout: `docs/MBM_GEN_PAIRING_FINAL_CLOSEOUT.md` — owner claimed the remaining 7 pairings done, but **live GEN still shows those 7 failing** (8/15 verified; phase **not** complete). Note: Vios Fin/Minox 0.1%/5% appears on GEN CP `Raw7mUkuzzhVdAo88jpL` (Minoxidil Topical), not locked Dual Combo `BboYS4a2Uj7APetrFo6W`. Next creates preflight-only: `docs/MBM_GEN_NEXT_CREATE_PREFLIGHT.md` (13) — do **not** execute creates / do **not** authorize CREATE phase until pairing closeout is YES.
