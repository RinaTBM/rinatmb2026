...
- Registry: `pairingVerificationRegistry.ts` → apply via `applyPairingVerification.ts` (also promotes `GEN_PAIRING_PENDING` → `ROUTING_READY` for verified CPs only). Latest live closeout: `docs/MBM_GEN_PAIRING_FINAL_CLOSEOUT.md` — **8/15** verified; phase **not** complete. **Discrepancy diagnostic:** `docs/MBM_GEN_SCRIPTful_PAIRING_DISCREPANCY_DIAGNOSTIC.md` — owner Scriptful/GEN UI edits may have landed on **different Client Products** than website-locked IDs (esp. Minoxidil → `Raw7m…`, TIR options → GLP-2 `SvFDJ7…`, Wolverine capsule med → injection CP). Do **not** assume failed save; do **not** retarget routing without owner review. Next creates preflight-only: `docs/MBM_GEN_NEXT_CREATE_PREFLIGHT.md` (13).
