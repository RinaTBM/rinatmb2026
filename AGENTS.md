...
- **GEN/Whop checkout (WHOP-2, cutover OFF):** optional path for mapped one-time Rx → GEN Hosted Checkout → Whop. Authoritative flag `GEN_WHOP_CHECKOUT_ENABLED` defaults **false**; Vite `VITE_GEN_WHOP_CHECKOUT_ENABLED` is a UI hint only. Accessories + SEM/TIRZ membership stay on Tagada. Mapping: `gen_whop_checkout_map`. Correlation: `gen_checkout_sessions`. Edge: `create-gen-whop-checkout-session`, `get-gen-checkout-status`, `reconcile-gen-whop-checkout` (server-side payment authority — browser return never marks paid). See `docs/GEN_WHOP_CHECKOUT_ROUTING.md`. Do **not** enable production cutover without owner approval. Staging (`mxvaxkkwrbwhqasnsjpm`) may have WHOP-2 tables/functions while production stays untouched.
