...
- Registry: `pairingVerificationRegistry.ts` → apply via `applyPairingVerification.ts` (also promotes `GEN_PAIRING_PENDING` → `ROUTING_READY` for verified CPs only). Latest live re-read: `docs/MBM_GEN_PAIRING_POSTCHECK_3.md` (8 verified CPs; existing pairing phase **not** complete). Next creates are preflight-only in `docs/MBM_GEN_NEXT_CREATE_PREFLIGHT.md` (TIR×8 + Estradiol patches×4 + NAD r85×1) — do **not** execute creates until owner says so.
