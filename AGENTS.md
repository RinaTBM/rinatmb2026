...
- Registry: `pairingVerificationRegistry.ts` → apply via `applyPairingVerification.ts` (also promotes `GEN_PAIRING_PENDING` → `ROUTING_READY` for verified CPs only). Latest live closeout: `docs/MBM_GEN_PAIRING_FINAL_CLOSEOUT.md` — **8/15** verified; phase **not** complete. Discrepancy diagnostic: `docs/MBM_GEN_SCRIPTful_PAIRING_DISCREPANCY_DIAGNOSTIC.md`. **Owner fix plan:** `docs/MBM_GEN_FINAL_MANUAL_FIX_PLAN.md` — 6 direct GEN admin fixes (keep locked CPs; TIR membership ≠ GLP-2 `SvFDJ7…`); Minoxidil recommendation **KEEP_BBOYS_AND_ATTACH** (Raw7m not clean — Tretinoin + plain Minoxidil mismatches). Do **not** change routing/registry until owner completes fixes + re-audit. Next creates: `docs/MBM_GEN_NEXT_CREATE_PREFLIGHT.md` (13) unchanged.
