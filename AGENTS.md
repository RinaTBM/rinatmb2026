...
- **Tagada ShippingRates must remain disabled** for this architecture. Simple Checkout (`funnelv2_834831bf5aa7` on `checkout.mybaremethod.com`) still embeds a Studio **ShippingRates** island on checkout page `checkout-5596ip` (plugin `checkout-6d366d`). Funnel API JSON does not include that island — remove it in Tagada Studio and republish. Until then keep `TAGADA_SHIPPING_PARITY_BLOCKER` and FE eligibility blocking `shipping_cents > 0`.
