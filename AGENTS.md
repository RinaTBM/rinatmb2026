...
- **GEN medication retail pricing (owner-locked, GEN-CATALOG-1H):** `raw = (medicationCost × 1.75) + pharmacyShipping`, then round to **nearest whole-dollar ending in 9** (equidistant → round up). Customer medication shipping is included in retail — do **not** add pharmacy shipping again at website checkout. Do **not** use workbook `Cost +75%` (that is landed×1.75). Website/GEN list prices are reference only. Multiple formulary cost bases → `MULTIPLE_COST_BASIS_REVIEW` (no silent choice). Authority doc: `docs/GEN_FINAL_MBM_RETAIL_PRICING.md`.
