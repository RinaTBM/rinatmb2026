...
> **Bolt note:** Bolt commits `8180b17` and `0ccdf22` deleted these files from the deploy branch. Flat-rate UI was re-applied on top of `0ccdf22` (no history rewrite). If Bolt preview again shows only Auto-Refill + One-Time (or a 15% “Active Wellness Membership” CTA), pull the latest `deploy/my-bare-method-integrated-2026` tip that restores `src/lib/pricing/weightMembership.ts` + ProductPage flat-rate UI. Do not let Bolt regenerate `purchaseOptions.ts` or `ProductPage.tsx` without this feature.
