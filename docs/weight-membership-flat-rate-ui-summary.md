...
> **Bolt note:** A Bolt commit (`8180b17`) previously deleted these files from the deploy branch. If Bolt preview again shows only Auto-Refill + One-Time (or a 15% “Active Wellness Membership” CTA), re-sync / pull the latest `deploy/my-bare-method-integrated-2026` commit that restores `weightMembership.ts` + ProductPage flat-rate UI. Do not let Bolt regenerate `purchaseOptions.ts` or `ProductPage.tsx` without this feature.
