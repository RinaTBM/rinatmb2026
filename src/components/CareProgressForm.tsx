...
export function CareProgressForm({ checkoutUrl, onContinue }: { checkoutUrl: string; onContinue: () => void }) {
  ...
  return <form onSubmit={save} className="mt-4 space-y-4">
    ...
    <label className="flex items-start gap-3 text-sm"><input type="checkbox" checked={smsConsent} onChange={e => setSmsConsent(e.target.checked)} className="mt-1" />I agree to automated My Bare Method texts about unfinished purchases or intake. Up to 3 texts per step. Msg &amp; data rates may apply. Reply STOP to opt out or HELP for help. Consent is not required to purchase.</label>
