 ...
 * Mirror kept in supabase/functions/_shared/tagadaWebhookCorrelation.ts.
...
/**
 * Live Tagada webhooks nest commerce under `data` and put
 * `mbmOrder:<ORDER>` inside `data.customer.tags`.
 */
...
/** Correlation plan — never includes email. */
export type CorrelationStep =
  | 'mbmOrder_tag'
...
export function planTagadaOrderCorrelation(payload: Record<string, unknown>): {
  orderNumber: string | null;
  ids: TagadaExternalIds;
  primaryStep: CorrelationStep;
} {
  const orderNumber = extractOrderNumberFromTagadaPayload(payload);
  const ids = extractExternalIdsFromTagadaPayload(payload);
  if (orderNumber) {
    return { orderNumber, ids, primaryStep: 'mbmOrder_tag' };
  }
  if (ids.externalCheckoutSessionId) {
    return { orderNumber: null, ids, primaryStep: 'external_checkout_session_id' };
  }
  if (ids.externalPaymentId) {
    return { orderNumber: null, ids, primaryStep: 'external_payment_id' };
  }
  if (ids.externalOrderId) {
    return { orderNumber: null, ids, primaryStep: 'external_order_id' };
  }
  return { orderNumber: null, ids, primaryStep: 'unmatched' };
}
