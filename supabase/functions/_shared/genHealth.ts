/**
 * GEN Health V2 client + pure helpers (Edge / Deno).
 * Mirror of src/lib/genHealth/genHealth.ts — keep in sync.
 * Outbound HTTP gated by GEN_HEALTH_ENABLED — default false.
 */
...
export async function createOrReuseGenPatient(
  ...
  const patient: Record<string, unknown> = { email: input.email };
...
export async function createGenOrder(
  ...
  // GEN V2 order create shape (confirmed via staging probe):
  // { patient_id, order: { clientProductId, transactionId } }
  // payment_status requires GEN "API Orders" — omit unless env enabled.
  ...
  if (String(env.GEN_API_ORDERS_PAYMENT_STATUS_ENABLED || "").toLowerCase() === "true") {
...
function extractPharmacyAndPrescriptionFromOrder(order: GenOrderResponse): {
  ...
  const rx =
    (raw.prescription && typeof raw.prescription === "object"
      ...
      : null) ||
    (Array.isArray(raw.prescriptions) && raw.prescriptions[0] &&
        typeof raw.prescriptions[0] === "object"
  ...
  const ship =
    ...
    (raw.pharmacy && typeof raw.pharmacy === "object"
      ...
      : null);
  return {
    genPrescriptionId: pickStr(
      rx?.id,
      raw.prescriptionId,
      raw.prescription_id,
      raw.genPrescriptionId,
    ),
 ...
 * Does not map into MBM fulfillment.
 */
...
/** Membership rebill must never create GEN medication orders. */
