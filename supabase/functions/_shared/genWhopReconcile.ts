/**
 * Edge mirror of src/lib/payments/genWhopReconcile.ts — keep in sync.
 */
...
export function evaluateGenWhopReconcile(input: GenWhopReconcileInput): GenWhopReconcileResult {
  ...
  if (input.whopPayments.length === 0 && input.genOrders.length === 0) {
    return {
      ok: false,
      action: "pending",
      sessionStatus: input.currentSessionStatus === "created" ? "pending" : "processing",
...
export function normalizeWhopAmountToCents(amount: unknown): number | null {
...
export function mapWhopPaymentRecord(raw: Record<string, unknown>): GenWhopPaymentEvidence | null {
  ...
  const amountRaw =
    raw.total || raw.amount || raw.final_amount || raw.usd_total || plan?.initial_price || null;
  const currency =
  ...
  return {
    ...
    whopCheckoutConfigId:
      ...
      null,
    status,
    amountCents: normalizeWhopAmountToCents(amountRaw),
    currency,
