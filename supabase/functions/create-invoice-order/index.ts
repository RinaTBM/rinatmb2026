...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const allowedShipping = new Set(["accessory", "two_day", "next_day", "free_over_500", "none"]);
    if (!allowedShipping.has(requestedShippingMethod) || requestedShippingMethod === "standard") {
      return json({ error: "Invalid shipping method. Choose Accessory, Two-Day, or Next-Day shipping." }, 400);
    }
