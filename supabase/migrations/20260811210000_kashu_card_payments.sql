...
-- SKU → Tagada product/variant/price mapping (products must be created in Tagada first)
create table if not exists public.kashu_sku_map (
  ...
  tagada_price_id text,
  mbm_price_cents integer,
  tagada_price_cents integer,
  is_active boolean not null default true,
  notes text,
  updated_at timestamptz not null default now()
);

-- Additive upgrades if an earlier draft of this table already exists
alter table public.kashu_sku_map add column if not exists mbm_product_id text;
alter table public.kashu_sku_map add column if not exists tagada_price_id text;
alter table public.kashu_sku_map add column if not exists mbm_price_cents integer;
alter table public.kashu_sku_map add column if not exists tagada_price_cents integer;

create unique index if not exists kashu_sku_map_tagada_variant_id_uidx
  on public.kashu_sku_map (tagada_variant_id)
  where tagada_variant_id is not null and is_active = true;

comment on table public.kashu_sku_map is
  'Maps MBM variant SKUs to Tagada product/variant/price IDs for hosted checkout. Do not replace MBM IDs.';
...
comment on column public.kashu_sku_map.tagada_price_id is
  'Tagada priceId for subscriptions/recurring catalog reference.';

-- Service-role only: revoke client roles and enable RLS with no client policies.
...
revoke all on table public.kashu_sku_map from anon, authenticated;
alter table public.payment_webhook_events enable row level security;
alter table public.kashu_sku_map enable row level security;
